...
@Component({
  selector: 'app-complete-legal-advice',
  templateUrl: './complete-legal-advice.component.html',
  styleUrls: ['./complete-legal-advice.component.css']
})
export class CompleteLegalAdviceComponent implements OnInit {
  ...
  getlegalAdvice() {
    this.requestParamModel.token = sessionStorage.getItem("authToken") || '';
    this.requestParamModel.flag = sessionStorage.getItem("role") || '';
    this.legalService.getCompleteLegalMessage(this.requestParamModel).subscribe((resp: any) => {
      if (resp.code === 1) {
        const dataList = JSON.parse(JSON.stringify(resp));
        if (resp.code === 1) {
          dataList.data[0].forEach((eachRow: OrderRequest) => {
